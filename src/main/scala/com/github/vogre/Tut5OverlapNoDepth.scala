...
class Tut5OverlapNoDepth extends Tutorial {
  
  override val name = "Tutorial 5 Overlap No Depth"
  ...
  def initializeVertexArrayObjects {
    ...
    val secondObjectOffset = 4 * 3 * (numberOfVertices / 2)
    val secondObjectColors = colorOffset + 4 * 4 * (numberOfVertices / 2)
    
    //Use the same buffer object previously bound to GL_ARRAY_BUFFER.	
    glEnableVertexAttribArray(0)
	glEnableVertexAttribArray(1)
    glVertexAttribPointer(0, 3, GL_FLOAT, false, 0, secondObjectOffset)
    glVertexAttribPointer(1, 4, GL_FLOAT, false, 0, secondObjectColors)
